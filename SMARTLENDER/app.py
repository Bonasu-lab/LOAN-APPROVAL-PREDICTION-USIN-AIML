...
def predict(name, gender, married, education, employed, income, loan_amount, tenure):
    try:
        ...
        return "<p style='color:red'>❌ Please fill all fields</p>", "₹0", "₹0K", "₹0", 650, 0, None
    ...
    if eligibility >= 70:
        status = "PRE-APPROVED"
        status_html = f"<div style='padding:35px; border-radius:20px; background:linear-gradient(135deg, #10B981 0%, #059669 100%); color:white; text-align:center; box-shadow:0 15px 40px rgba(16,185,129,0.4);'><h2>✅ LOAN {status}</h2><h3>Congratulations!</h3><p><b>Eligibility:</b> {eligibility}% | <b>Credit Score:</b> {credit_score}</p></div>"
    else:
        status = "REJECTED"
        status_html = f"<div style='padding:35px; border-radius:20px; background:linear-gradient(135deg, #EF4444 0%, #DC2626 100%); color:white; text-align:center; box-shadow:0 15px 40px rgba(239,68,68,0.4);'><h2>❌ LOAN {status}</h2><h3>Not eligible</h3><p><b>Eligibility:</b> {eligibility}% | <b>Credit Score:</b> {credit_score}</p></div>"

    pdf_file = generate_pdf(name, eligibility, status)
...
with gr.Blocks(title="Smart Lender Pro") as demo:
    ...
    # ===== 1. FORM PAINA =====
    with gr.Column():
        gr.HTML("<h3 style='color:#60A5FA; text-align:center;'>📝 Enter Your Details</h3>")
        ...
        predict_btn = gr.Button("⚡ Get Instant Decision", variant="primary", size="lg")

    # ===== 2. RESULT KINDHA - DESIGN SAME =====
    with gr.Row():
    ...
    predict_btn.click(fn=predict, inputs=[name, gender, married, education, employed, income, loan_amount, tenure], outputs=[out_status, card1, card2, card3, credit_gauge, credit_gauge, download_btn])
